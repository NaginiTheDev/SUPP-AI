...
const DEFAULT: Omit<CustomerProfile, "goal"> = {
  gender: "male",
  age: 25,
  heightCm: 175,
  weightKg: 75,
...
export function CoachPanel({
  ...
  // Let a parent react to phase changes (e.g. expand layout when the stack lands).
  useEffect(() => {
    onPhaseChange?.(status);
  }, [status, onPhaseChange]);

  const set = (patch: Partial<CustomerProfile>) => setP((prev) => ({ ...prev, ...patch }));
  ...
  // ---- Pick inner content by phase. One persistent Shell animates its width,
  //      and a keyed wrapper cross-fades whenever the phase changes. ----------
  const inner =
    status === "result" && result ? (
      ...
      <div className="flex h-full flex-col items-center justify-center gap-5 px-8 text-center">
        <div className="relative h-16 w-16">
          <div className="absolute inset-0 animate-ping rounded-full bg-lime-400/30" />
          <div className="relative flex h-16 w-16 items-center justify-center rounded-full bg-lime-400 text-3xl">
            💪
          </div>
        </div>
        <div>
      ...
      <div className="flex h-full flex-col items-center justify-center gap-4 px-8 text-center">
        ...
        <button
          ...
          className="rounded-xl bg-lime-400 px-5 py-2.5 text-sm font-bold text-zinc-950"
        >
          Try again
        </button>
      </div>
      ...
      <div className="flex h-full flex-col">
        ...
        <div className="border-b border-zinc-800 px-6 pb-4 pt-5">
          <div className="flex items-center gap-2.5">
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-lime-400 text-lg">
              💪
            </div>
            <div className="leading-tight">
              <p className="text-sm font-black text-white">AI COACH</p>
              <p className="text-[11px] text-zinc-400">Protein House · Stack Builder</p>
            </div>
          ...
          <p className="mt-3 text-xs font-bold uppercase tracking-wider text-lime-400">
            {STEPS[step]}
          </p>
        </div>
            ...
            <div className="space-y-6">
              ...
              <Field label="Anything else Coach should know?" hint="Optional — allergies, injuries, supplements you already take, brands you like…">
                <textarea
                  ...
                  rows={3}
                  placeholder="e.g. allergic to shellfish, dodgy knees, already take a multivitamin, prefer Optimum Nutrition…"
                  className="w-full resize-none rounded-2xl border border-zinc-800 bg-zinc-900 px-3.5 py-3 text-sm text-white placeholder:text-zinc-600 outline-none focus:border-lime-400"
                />
          ...
          {step === 4 && (
            <Field label="Monthly budget" hint="What you'd like to spend on supplements">
              <div className="mb-3 text-center">
                <span className="text-4xl font-black text-white">Rs {p.budget?.toLocaleString()}</span>
              </div>
              <input
                type="range"
                min={500}
        ...
        <div className="flex gap-2 border-t border-zinc-800 p-4">
          ...
          {!isLast ? (
            <button
              onClick={() => setStep((s) => s + 1)}
              disabled={!canNext}
              className="flex-1 rounded-xl bg-lime-400 py-3 text-sm font-black text-zinc-950 transition hover:bg-lime-300 disabled:opacity-30"
            >
              Continue
            </button>
          ) : (
            <button
              onClick={generate}
              className="flex-1 rounded-xl bg-lime-400 py-3 text-sm font-black text-zinc-950 transition hover:bg-lime-300"
            >
              Build my stack 💪
            </button>
          )}
      ...
      {options.map((o) => (
        <button
          ...
          className={`flex-1 rounded-xl py-2.5 text-sm font-bold transition ${
            value === o.value ? "bg-lime-400 text-zinc-950" : "text-zinc-400 hover:text-white"
          }`}
        >
          {o.label}
        </button>
