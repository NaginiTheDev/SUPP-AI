...
import { ProtinCoachPanel, type CoachPhase } from "./CoachPanel";
...
export function CoachExperience() {
  ...
  return (
    <div className={`mx-auto grid min-h-screen max-w-6xl items-center px-6 py-12 transition-[grid-template-columns,gap] duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] ${expanded ? "gap-0 lg:[grid-template-columns:0fr_1fr]" : "gap-10 lg:[grid-template-columns:1fr_1fr]"}`}>
      ...
      <div className="order-1 lg:order-2"><ProtinCoachPanel onPhaseChange={onPhaseChange} /></div>
    </div>
  );
}
...
function Pitch() {
  return (
    <div>
      <span className="inline-block rounded-full bg-sky-400 px-3 py-1 text-xs font-black text-blue-950">protin.is · {t("appName")}</span>
      <h1 className="mt-5 text-4xl font-black leading-[1.05] tracking-tight sm:text-6xl">{t("intro_title")}</h1>
      ...
      <ul className="mt-6 space-y-2.5 text-sm text-zinc-300">
        {[t("intro_b1"), t("intro_b2"), t("intro_b3")].map((line) => (
          <li key={line} className="flex items-center gap-2.5">
            <span className="flex h-5 w-5 items-center justify-center rounded-full bg-sky-400 text-xs font-black text-blue-950">✓</span>
            {line}
          </li>
        ))}
      </ul>
